"""Similar fusion_classification_network_cat, but with extra input/output
for dummy memory. Also force midfusion concat and feature_dim
"""
from .fusion_classification_network import Fusion_Classification_Network


class ActregFc(Fusion_Classification_Network):
    """Similar fusion_classification_network_cat, but with extra input/output
    for dummy memory
    """
    def __init__(self, device, feature_dim, modality, num_class, num_segments,
        ...
        self.device = device
        self._input_dim = feature_dim

        # Replace fc1
        self._add_audiovisual_fc_layer(feature_dim, 1024)

        # Replace fc_action/fc_verb+fc_noun
        self._add_classification_layer(1024)

    def forward(self, inputs, hidden=None):
        """Wrapper of the parent forward because we assume the inputs here are
        already concatenated. Hidden is only to make the caller happy
        """
        if inputs.ndim == 3:
